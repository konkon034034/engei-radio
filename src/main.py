    ...
    for keyword in shuffled_keywords:
        try:
            ...
            resp = requests.get("https://www.googleapis.com/youtube/v3/search", params={
                ...
                'relevanceLanguage': 'ja', 'maxResults': 15, 'key': api_key,
                    ...
                    if v['video_id'] in stats_map:
                        ...
                        if views >= min_views:
                            enriched_videos.append({
                                ...
                                'source': 'YouTube',
                                'views': f"{views:,}回再生", 'views_count': views,
                                'comments': f"{comments:,}件コメント", 'comments_count': comments,
...
- 動画で使えるネタ: [1行]"""
        ...
        if resp.status_code == 200:
            ...
            if output_text:
                ...
                print(f"  [WARN] X投稿テキスト抽出失敗")
        elif resp.status_code == 429:
            print(f"  [WARN] Grok APIレート制限。X検索終了")
        else:
...
def fetch_news_from_rss(keywords, max_articles=5):
    ...
    for keyword in keywords:
        try:
            ...
            rss_response = requests.get(rss_url, timeout=15, headers={
...
def summarize_youtube_for_script(videos, channel_theme="年金"):
    ...
    """
    if not videos:
        return f"市民生活をされている方々の暮らしぶりを紹介します"
...
def generate_text_image(text, font_size, text_color, border_color, border_width, is_bold=False, size=None, align="left"):
    ...
    if size and W > 0:
        ...
        for line in lines:
            ...
            draw_text_bold_with_border(draw, line, (x_text, y_text), font, text_color, border_color, border_width, is_bold)
...
class VideoEngineV4:
    ...
    def _get_api_keys(self):
        ...
        if not keys: raise Exception("APIキーが設定されていません")
        return keys
    ...
    def generate_content(self):
        ...
        print(f"--- 台本・メタデータ生成開始 (モデル: gemini-2.0-flash) ---")
        print(f"[API] Gemini API にリクエスト送信（長時間待機モード: 最大300秒）")
        ...
        yt_videos = fetch_trending_youtube_videos(
            ...
            max_videos=8, days=14, min_views=300
        )
...
{stats_section}{x_section}"""
        ...
        # ----- 第1段階: 構成生成（人間ドキュメンタリー仕様） -----
        print("===== 第1段階: 構成生成（人間ドキュメンタリー） =====")
        structure_prompt = f"""
...
- section: always "main"
- emotion: default/question/surprised/thinking/happy/concerned
...
"""
            ...
            raw_text = call_llm_with_fallback(
                messages=[
                    {"role": "system", "content": f"Generate detailed script. script must have {min_lines}+ lines. Output JSON."},
        ...
        ending_ng_words = ["今日はここまで", "ここまでです", "次回をお楽しみ", "また次回", "また来週", "またね", "さようなら", "バイバイ"]
        ...
        for i, (idx1, line1) in enumerate(honne_lines):
            for idx2, line2 in honne_lines[i+1:]:
                ...
                if similarity_ratio(text1, text2) > 0.6:
                    issues_found.append(f"[B] 本音重複 (行{idx1+1}と行{idx2+1}): 類似度{similarity_ratio(text1, text2):.0%}")
            ...
            for i, (idx1, line1) in enumerate(news_lines):
                for idx2, line2 in news_lines[i+1:]:
                    ...
                    if similarity_ratio(text1, text2) > 0.5:
                        issues_found.append(f"[C] ニュース重複 (行{idx1+1}と行{idx2+1}): 類似度{similarity_ratio(text1, text2):.0%}")
        
        # D: 挨拶パターン検出（冒頭以外）- 長い順に処理し包摂問題を回避
        # 「皆さん、こんにちは」⊃「こんにちは」なので長い順にマッチ→除去→text更新
        greeting_ng_patterns = sorted(["こんにちは", "皆さん、こんにちは", "今日のニュースです", "本日のニュースは", "ニュースをお届け"], key=len, reverse=True)
...
"""
        ...
        title = call_llm_with_fallback(
            messages=[
                {"role": "system", "content": "サムネイルタイトルを2行で出力してください。各行6文字以内厳守。全力で攻めるフック。未確定なら「！？」で逃げ道を作る。"},
                {"role": "user", "content": prompt}
            ],
            max_tokens=50,
            temperature=0.8
    ...
    def generate_youtube_thumbnail(self, title, comment=None, script=None):
        """
        YouTube投稿用サムネイル画像を生成（v7仕様: 曜日カラバリ+感情連動アイコン）
        ...
        Returns:
            str: サムネイル画像のパス
        """
        print("--- YouTubeサムネイル画像生成中 (v7仕様: 曜日カラバリ+感情連動) ---")
        import random
        from datetime import datetime
        from PIL import Image, ImageDraw, ImageFont

        # ====== 曜日別カラーマップ ======
        # 月=白, 火=オレンジ, 水=青, 木=茶, 金=黄, 土=青, 日=赤
        weekday = datetime.now().weekday()  # 0=月, 1=火, ..., 6=日
        day_color_map = {
            0: {"title": (255, 255, 255), "outline": (60, 60, 80), "shadow": (30, 30, 50)},      # 月: 白文字+濃紺縁
            1: {"title": (255, 140, 0),   "outline": (80, 30, 0),  "shadow": (50, 20, 0)},       # 火: オレンジ+茶縁
            2: {"title": (30, 120, 255),  "outline": (255, 255, 255), "shadow": (10, 40, 100)},   # 水: 青+白縁
            3: {"title": (160, 100, 40),  "outline": (255, 255, 255), "shadow": (60, 30, 10)},    # 木: 茶+白縁
            4: {"title": (255, 220, 0),   "outline": (80, 50, 0),  "shadow": (60, 40, 0)},       # 金: 黄+濃茶縁
            5: {"title": (50, 100, 200),  "outline": (255, 255, 255), "shadow": (20, 40, 80)},    # 土: 青+白縁
            6: {"title": (220, 30, 30),   "outline": (255, 255, 255), "shadow": (80, 10, 10)},    # 日: 赤+白縁
        }
        ...
        if script:
            ...
            # neutral以外で最多のカテゴリを選択（同数ならguts優先）
        ...
        # カツミとヒロシからランダムに1人選び、感情に合ったアイコンを使う
        ...
        draw.rounded_rectangle(
            ...
            radius=50, fill=(255, 255, 255), outline=(100, 100, 100), width=4
        )
        ...
        draw.line([(bubble_x + 40, bubble_y + bubble_h//2 + 20), (bubble_x + 80, bubble_y + bubble_h - 20)], 
                  fill=(255, 255, 255), width=8)

        # ==== レイヤー3: 吹き出し内テキスト（108pt） ====
        if comment:
        ...
        for j, line in enumerate(comment_lines):
            ...
            draw.text(
                ...
                line, fill=colors["bubble_text"], font=comment_font
            )

        # ==== レイヤー4: タイトル（参考画像準拠レイアウト） ====
        # 1段目: 赤帯位置・左端から開始 (x=20)
        # 2段目: 黄帯位置・キャラ右側から開始 (x=350)
        title_lines = title.split('\n')[:2]
        y_positions = [-10, 175]  # 1行目(赤帯), 2行目(黄帯)
        x_positions = [20, 350]   # 1行目(左端), 2行目(キャラ右)

        for j, line in enumerate(title_lines):
            x_pos = x_positions[j] if j < len(x_positions) else x_positions[-1]
            draw_text_with_outline(
                draw, (x_pos, y_positions[j] if j < len(y_positions) else y_positions[-1] + 190), line, title_font,
                colors["title"], colors["title_outline"],
        ...
        img.save(out_path)
        print(f"[OK] v7サムネイル保存完了: {out_path} (icon: {icon_name})")
        return out_path
        ...
        try:
            ...
            if os.path.exists(mp3_path) and os.path.getsize(mp3_path) > 0:
                ...
                if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
                    ...
                    print(f"[WARN]  Edge TTS: WAV変換失敗")
                    return False
            else:
                print(f"[WARN]  Edge TTS: MP3生成失敗")
                return False
    ...
    def create_video_with_remotion(self, content, audio_path):
        ...
        if audio_duration > 0 and individual_total > 0:
            ...
            print(f"[Anti-Drift] 個別累計: {individual_total:.2f}秒, 連結音声: {audio_duration:.2f}秒, 補正比率: {correction_ratio:.4f}")
        else:
        ...
        if script_with_frames and audio_duration > 0:
            ...
            print(f"[Anti-Drift] 最終行endFrameを{int(audio_duration * fps) + slide_duration_frames}に物理固定（slideDuration={slide_duration_frames}f加算済）")
        
        # 全体のdurationInFramesは音声実測値 + slideDuration
        total_frames = (int(audio_duration * fps) if audio_duration > 0 else int(accumulated_duration * fps)) + slide_duration_frames
        ...
        def extract_chart_data(text, chart_idx=0):
            ...
            def detect_chart_type_from_context(text_context, is_pct=False):
                """文脈から最適なチャートタイプを判定"""
                if re.search(r'増額|増加|引き上げ|引上げ|アップ|上が|減額|減少|引き下げ|切り下げ|ダウン|下が|増え|減り|前年比|比べ|変更|改定', text_context):
                    return "bar"
            ...
            def is_negative_context(text_context):
                ...
                return bool(re.search(r'減額|減少|引き下げ|ダウン|下が|下落|減り|赤字|損|不足|マイナス|負担|削減|カット|廃止', text_context))
            ...
            if pct_match:
                ...
                if ctype == "bar":
                    result = {"type": "bar", "label": label, "value": 100 + val if '増' in text or '上' in text or 'アップ' in text else 100 - val, "unit": "%", "maxValue": max(150, 100 + val + 20), "negative": negative}
                elif ctype == "compare":
                    result = {"type": "compare", "label": label, "value": val, "unit": "%", "maxValue": 100, "compareValue": 100 - val, "compareLabel": "その他", "negative": negative}
                else:
            ...
            if wari_match:
                ...
                negative = is_negative_context(text)
                if ctype == "compare":
                    result = {"type": "compare", "label": label, "value": val, "unit": "%", "maxValue": 100, "compareValue": 100 - val, "compareLabel": "その他"}
                else:
            ...
            if money_man:
                val_str = money_man.group(1).translate(str.maketrans('０１２３４５６７８９，', '0123456789,')).replace(',', '')
            ...
            if money_match:
                val_str = money_match.group(1).translate(str.maketrans('０１２３４５６７８９，', '0123456789,')).replace(',', '')
                val = int(val_str)
                if val >= 100:
            ...
            if people_match:
                val_str = people_match.group(1).translate(str.maketrans('０１２３４５６７８９，', '0123456789,')).replace(',', '')
                val = int(val_str)
        ...
        if polls:
            ...
            main_lines = [l for l in script_with_frames if l.get('section') not in ('hikaeshitsu', 'hikaeshitsu_jingle', 'ending', 'opening', 'opening_jingle')]
            if main_lines:
                ...
                for pi, poll in enumerate(polls):
                    ...
                    print(f"[OK] poll[{pi}] \"{question[:20]}\" -> 行{best_idx}(score={best_score:.1f}, frame={trigger_frame})")
        ...
        print(f"[OK] chartData合計: {len(chart_data_list)}件（poll: {len(polls)}件 + 数値: {len(chart_data_list) - len(polls)}件）")
        ...
        household_budget_data = {
            "personLabel": _random.choice([
                "73歳女性・一人暮らし", "68歳男性・妻と二人暮らし",
                "75歳女性・団地暮らし", "70歳夫婦・都内在住",
                "66歳女性・パート兼業", "72歳男性・持ち家あり",
            ]),
        ...
        if household_budget_data["expenses"][0]["amount"] == 0:
            household_budget_data["expenses"][0] = {"label": "固定資産税等", "amount": _random.choice([5000, 8000, 12000])}
        ...
        cmd = [
            "npx", "remotion", "render",
            "src/index.ts", "DynamicNewsVideo",
            output_path,
            "--props", props_json,
            "--concurrency", "2"  # 並列処理数を制限して安定化
        ]
    ...
    def create_slide_video(self, slide_image, jingle_audio, output_path, duration=3):
        ...
        slide_path = os.path.join(public_dir, slide_image) if os.path.exists(os.path.join(public_dir, slide_image)) else os.path.join(assets_dir, slide_image)
        jingle_path = os.path.join(public_dir, jingle_audio) if os.path.exists(os.path.join(public_dir, jingle_audio)) else os.path.join(assets_dir, jingle_audio)
    ...
    def generate_quiz_intro_video(self, chart_data_list):
        ...
        try:
            ...
            # 「1位は何でしょう？」
            draw.text((width // 2, 220), "あなたは正解できますか？見逃すと損かも...", fill=(255, 150, 50), font=small_font, anchor="mt")
            ...
            for idx, item in enumerate(quiz_items):
                ...
                if label == "？？？":
                    ...
                    for bx in range(0, bar_width, 20):
                        ...
                        draw.rectangle([bar_left + bx, y, bar_left + bx + segment_w, y + bar_height], fill=(255, 100, 100, 150))
                else:
                ...
                if label != "？？？":
                    draw.text((bar_left + bar_width + 20, y + bar_height // 2), value_text, fill=(200, 200, 200), font=small_font, anchor="lm")
                else:
                    draw.text((bar_left + bar_width + 20, y + bar_height // 2), "??%", fill=(255, 200, 50), font=small_font, anchor="lm")
            
            # 「正解は本編で！」
            draw.text((width // 2, height - 120), "答えを知らないと損するかも...最後まで見てね！", fill=(255, 200, 50), font=title_font, anchor="mt")
...
'''
        ...
        try:
            ...
            text = call_llm_with_fallback(
                messages=[
                    {'role': 'system', 'content': 'あなたはYouTube台本作家です。JSON配列形式で台本を出力してください。控室トークはこの番組のメインコンテンツです。カツミとヒロシの人間性を全開にして、視聴者が「よくぞ言ってくれた！ファンになった！」と思う長めの本音トークを書いてください。1発言は40文字以内で書いてください。'},
                    {'role': 'user', 'content': prompt}
                ],
                max_tokens=3072,
                temperature=0.9
    ...
    def generate_ending_video(self):
        ...
        try:
            ...
            if os.path.exists(katsumi_path) and os.path.exists(hiroshi_path):
                ...
                char_height = 350
                katsumi_img = katsumi_img.resize((int(katsumi_img.width * char_height / katsumi_img.height), char_height))
                hiroshi_img = hiroshi_img.resize((int(hiroshi_img.width * char_height / hiroshi_img.height), char_height))
            ...
            cmd = [
                'ffmpeg', '-y',
                '-loop', '1', '-i', ending_img_path,
                '-c:v', 'libx264', '-t', str(ending_duration),
                '-pix_fmt', 'yuv420p',
                '-an',
                ending_path
            ]
    ...
    def run(self, use_remotion=False):
        try:
            ...
            audio_retry = 0
            while self.mode not in ["--test", "--short-prod"] and audio_duration < MIN_DURATION_SECONDS and audio_retry < MAX_AUDIO_RETRIES:
                audio_retry += 1
            ...
            if self.mode not in ["--test", "--short-prod"] and audio_duration < MIN_DURATION_SECONDS:
                raise Exception(f"音声長不足: {audio_duration:.1f}秒 < {MIN_DURATION_SECONDS}秒（{MAX_AUDIO_RETRIES}回リトライ後も不足）")
...
def main():
    ...
    if args.script_only:
        ...
        print(f"台本JSON: output/content.json")
        return
